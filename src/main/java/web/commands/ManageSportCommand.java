...
public class ManageSportCommand extends CommandProtectedPage
{
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws UserException
    {
       ...
       if (deleteId != null)
       {
           int rowsAffected = bmiFacade.deletSport(Integer.parseInt(deleteId));
           if (rowsAffected > 0)
           {
       ...
       {
           //Updater sportgren i databasen
           String name = request.getParameter("name");
           String sportId = request.getParameter("sports_id");
           int rowsInserted = bmiFacade.updateSports(Integer.parseInt(sportId),name);
           if (rowsInserted == 1)
           {
               request.getServletContext().setAttribute("sportList", bmiFacade.getAllSports());
           }
           System.out.println("nyt navn: " + name);
